...
\begin{enumerate}
    ...
    \vspace{0.3cm} % Ajout d'un espace avant l'équation suivante
    ...
    \vspace{0.3cm} % Ajout d'un espace avant le texte explicatif
...
\begin{enumerate}
    ...
    \\
    \vspace{0.9cm}
    &= \frac{1 + \Theta_1 L^6 + \theta_1 L + \theta_1 \Theta_1 L^7}{1 - \Phi_1 L^6 - \phi_1 L + \phi_1 \Phi_1 L^7} \varepsilon_t
    \end{align*}

   \noindent Nous obtenons initialement un processus $ARMA(7,7)$. Cependant, les coefficients $\theta_1 \Theta_1$ et $\phi_1 \Phi_1$ étant négligeables, nous nous ramenons à un $ARMA(6,6)$.

Dans la partie $MA$, seuls $\theta_1$ et $\Theta_1$ sont présents, ce qui signifie que $4$ $(=6-2)$ coefficients sont nuls. Pour la partie $AR$, nous avons $\phi_1$ et $\Phi_1$, ce qui veut dire que $4$ $(=6-2)$ coefficients sont nuls.

Au final :
\begin{itemize}
\item Pour la partie $MA$ : 1 coefficient est négligeable et 4 sont nuls.
\item Pour la partie $AR$ : 1 coefficient est négligeable et 4 sont nuls.
\end{itemize}

Ainsi, il ne reste que 4 coefficients à estimer.  \\ \\ \\ \\
...
\begin{enumerate}
    ...
    \begin{align*}
    Y_t &= \frac{\left(1 + \Theta_1 L^{12}\right)}{\left(1 - \phi_1 L - \phi_2 L^2\right)} \varepsilon_t \\
    \vspace{0.9cm}
    &= \frac{1 + \Theta_1 L^{12}}{1 - \phi_1 L - \phi_2 L^2} \varepsilon_t
    \end{align*}

   \noindent Nous obtenons donc un processus $ARMA(2,12)$. Étant donné qu'il n'y a pas de coefficient négligeable, nous conservons le processus tel quel.

Dans la partie $MA$, seul $\Theta_1$ est présent, ce qui signifie que $11$ $(=12-1)$ coefficients sont nuls. Pour la partie $AR$, nous avons $\phi_1$ et $\phi_2$, ce qui veut dire qu'aucun $(=2-2)$ coefficient n'est nul.

Au final :
\begin{itemize}
\item Pour la partie $MA$ : aucun coefficient n'est négligeable et 11 sont nuls.
\item Pour la partie $AR$ : aucun coefficient n'est négligeable et aucun n'est nul.
\end{itemize}

Ainsi, il ne reste que 3 coefficients à estimer.  \\ \\ \\ \\
